...
const requiresAuthentication =
  require('../utils/middleware.js').requiresAuthentication;
const { packageAuthentication } = require('../utils/middleware');
...
packageRouter.get('/view', packageAuthentication, async (req, res) => {
  let pkgs = [];
  if (req.user.userName === 'alla') {
    pkgs = await Packages.getForAlla();
  } else {
    pkgs = await Packages.getAll();
  }
  ...
  averagePricePerItem =
    pricePerItemCollection.reduce((sum, price) => {
      return sum + price;
...
packageRouter.get('/view/:pkgId', packageAuthentication, async (req, res) => {
...
packageRouter.post('/edit/:pkgId', requiresAuthentication, async (req, res) => {
  ...
  if (successfulUpdate) {
    ...
    pkg.price_per_item =
      pkg.price && itemsInPackage.length > 0
...
packageRouter.get('/print/:pkgId', packageAuthentication, async (req, res) => {
  const pkgId = req.params.pkgId;
  const pkg = await Packages.findById(pkgId);
