...
itemRouter.post(
  ...
  upload.single('brandomania-picture'),
  async (req, res) => {
    try {
      ...
      dataObj.picture = `${removeSpaces(brand)}-${removeSpaces(
        type
      )}-${removeSpaces(colors)}-${tagNumber.toLowerCase()}.png`;
...
itemRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      dataObj.picture = `${removeSpaces(brand)}-${removeSpaces(
        type
      )}-${removeSpaces(colors)}-${dataObj.tag_number.toLowerCase()}.png`;
...
itemRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      let successfulDatabaseUpdate = await Clothing.updatePartial(
        itemId,
        dataObj
      );
...
itemRouter.get(
  '/view/:inventory/:gender/filtered',
  requiresAuthentication,
  async (req, res) => {
