...
const editSoldTo = async (itemId, soldTo) => {
  const UPDATE_SOLD_TO = "UPDATE clothing SET sold_to = $1 WHERE id = $2";
  let result = await dbQuery(UPDATE_SOLD_TO, soldTo, itemId);

  return result.rowCount > 0;
};

const editArrived = async (itemId, arrived) => {
  const UPDATE_ARRIVED = "UPDATE clothing SET arrived = $1 WHERE id = $2";
  let result = await dbQuery(UPDATE_ARRIVED, arrived, itemId);
