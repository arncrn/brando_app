...
seen, gender, style, picture, material, location, receipts.tax, packages.package_name
FROM clothing 
...
module.exports = class SessionPersistence {
  ...
  //     return getTotalCost(item, receipts);
  //   }
  // }
  ...
  async findItemById(id) {
    const ITEM = FIND_ITEM_TEXT + " WHERE id = $1";
  ...
  sortItems(items) {
    ...
    console.log(items[0].tag_number, 'line 39');
  ...
  async getWomensClothing() {
    const WOMENS_CLOTHING = FIND_ITEM_TEXT + " WHERE gender = 'women' ORDER BY tag_number";
    let result = await dbQuery(WOMENS_CLOTHING);
    return this.sortItems(result.rows);
  }

  async getMensClothing() {
    const MENS_CLOTHING = FIND_ITEM_TEXT + " WHERE gender = 'men' ORDER BY tag_number";
