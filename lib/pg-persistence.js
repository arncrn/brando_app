...
module.exports = class SessionPersistence {
  ...
  async loginUser(username, sessionID) {
    const LOGIN_USER = `UPDATE users 
      SET signed_in = true, session_id = $2
      WHERE username = $1`;
  ...
  async checkLoginStatus(sessionID) {
    const CHECK_STATUS = "SELECT signed_in FROM users WHERE session_id = $1";
  ...
  async getPendingWomensClothing() {
